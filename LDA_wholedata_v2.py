...
import pandas as pd
df = pd.read_csv('sample100000_normalized_v2.csv').drop_duplicates()
...
for topic_id, topic in lda_model.print_topics(num_topics=23, num_words=30):
    print('Topic #'+str(topic_id+1)+':')
    print(topic)
    print()
    
    
topics_coherences = lda_model.top_topics(bow_corpus, topn=23)
...
df.to_csv('topic_weight_LDA.csv')
